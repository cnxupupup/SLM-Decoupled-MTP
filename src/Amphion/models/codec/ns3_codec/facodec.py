...
class FACodecDecoder(nn.Module):
    ...
    def forward(
        ...
        if vq is True:
            ...
            x_timbre, spk_emb_all_layers = self.timbre_encoder(x_timbre, None, None)
            x_timbre = x_timbre.transpose(1, 2)
            spk_embs = torch.mean(x_timbre, dim=2)
            return outs, qs, commit_loss, quantized_buf, spk_embs, spk_emb_all_layers
...
class FACodecDecoderV2(nn.Module):
    ...
    def forward(
        ...
        if vq is True:
            if eval_vq:
                self.quantizer.eval()
            spk_emb_all_layers = None
            pads = torch.zeros([prosody_feature.shape[0], prosody_feature.shape[1], x.shape[-1] - prosody_feature.shape[-1]]).to(x.device)
            ...
            x_timbre = x_timbre.transpose(1, 2)
            x_timbre = self.timbre_encoder(x_timbre, None, None, return_in_layers=return_in_layers)
            if isinstance(x_timbre, tuple):
                x_timbre, spk_emb_all_layers = x_timbre
            x_timbre = x_timbre.transpose(1, 2)
            spk_embs = torch.mean(x_timbre, dim=2)
            return outs, qs, commit_loss, quantized_buf, spk_embs, spk_emb_all_layers
