...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x, key_padding_mask, condition=None, return_in_layers=False):
        spk_emb_all_layers = []
        ...
        for layer in self.layers:
            ...
            meaned = x.mean(dim=1)
            spk_emb_all_layers.append(meaned)
        if self.use_cln:
        ...
        return (x, spk_emb_all_layers[:-1]) if return_in_layers else x # delete the last one
